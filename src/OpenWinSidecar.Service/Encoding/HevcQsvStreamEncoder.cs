    ...
    public bool Initialize(int width, int height, int bitrateKbps = 4000)
    {
        lock (_syncLock)
        {
            ...
            try
            {
                ...
                //   -preset veryfast       NOTE: hevc_qsv accepts only its own preset names
                //                          (faster/veryfast/superfast...); x264-style names
                //                          like ultrafast make ffmpeg EXIT at startup, which
                //                          silently degrades the sink to JPEG at native res.
                ...
                //                          capture stall); QSV's small default buffer keeps
