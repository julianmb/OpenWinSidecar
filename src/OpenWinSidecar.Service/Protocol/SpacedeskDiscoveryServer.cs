...
public class SpacedeskDiscoveryServer : IDisposable
{
    private readonly int _port;
    private UdpClient? _udpListener;
    private CancellationTokenSource? _cts;

    public SpacedeskDiscoveryServer(int port = 28252)
    {
        _port = port;
    }
    ...
    private async Task ListenLoopAsync(CancellationToken token)
    {
        try
        {
            ...
            while (!token.IsCancellationRequested)
            {
                ...
                // Spacedesk discovery query or beacon
                var hostName = Environment.MachineName;
                var responsePayload = $"SPACEDESK_SERVER;NAME={hostName};PORT={_port};VER=1.0;DISPLAYS=1\n";
