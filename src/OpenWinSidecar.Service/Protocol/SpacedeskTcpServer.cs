...
public class SpacedeskTcpServer : IDisposable
{
    ...
    private async Task HandleWebSocketConnectionAsync(TcpClient client, NetworkStream stream, string request, CancellationToken token)
    {
        ...
        // Tell the client up front whether it must authenticate
        if (authToken != null)
        {
    ...
    /// <summary>
    /// Waits for the client's `auth:&lt;token&gt;` message and verifies it, allowing up to
    /// three attempts per connection (overall 6-second window) before closing. Sends
    /// `auth:ok` / `auth:denied` so the web client can prompt for the password.
    /// </summary>
    private static async Task<bool> AuthenticateSessionAsync(NetworkStream stream, SemaphoreSlim streamLock, string authToken, CancellationToken token)
    {
        ...
        try
        {
            ...
            for (int attempt = 1; attempt <= 3; attempt++)
            {
                ...
                if (FixedTimeStringEquals(presented, authToken))
                {
                    await WebCodecsFraming.SendTextAsync(stream, streamLock, "auth:ok", CancellationToken.None);
                    Console.WriteLine("[WebSocket] Session authenticated.");
                    return true;
                }
...
<body>
    ...
    <script>
        ...
        function submitAuth() {{
            ...
            if (pw && ws && ws.readyState === WebSocket.OPEN) ws.send('auth:' + pw);
        }}
