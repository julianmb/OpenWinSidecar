...
Download **OpenWinSidecar-Setup-0.2.0.exe** from [Releases](https://github.com/julianmb/OpenWinSidecar/releases/latest) and run it. The installer isn't code-signed yet, so SmartScreen may show "Windows protected your PC" — verify the SHA-256 published in the [release notes](https://github.com/julianmb/OpenWinSidecar/releases/latest) first, then choose **More info → Run anyway** (silent installs with `/SILENT` still need a one-time UAC approval — admin rights are required for the driver and firewall rule). The installer registers the virtual display driver (creating the real extra monitor), installs the Gyan FFmpeg full build via winget if no FFmpeg is present, adds the private-network firewall rule, and can be removed cleanly from Apps & Features. It is also submitted to winget as `julianmb.OpenWinSidecar` (with FFmpeg declared as a package dependency) — once that listing is approved:
