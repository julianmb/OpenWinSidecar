...
| [`src/OpenWinSidecar.Service/Protocol/SidecarTcpServer.cs`](file:///C:/Users/JulianB/source/repos/OpenWinSidecar/src/OpenWinSidecar.Service/Protocol/SidecarTcpServer.cs) | Multi-port HTTP/WebSocket server, HTML5/WebCodecs client, touch/input dispatcher. |
...
3. **No password is set by default** — the viewer connects straight away. To require one later: Console → Preferences → Access password → Save (auth then applies automatically, no restart needed).
...
0. **✅ Completed since this doc was written (September 2026 session)**:
   ...
   - **Access-password enforcement (security, optional)**: the `EncryptionPassword` registry setting gates everything **only when non-empty** — default is open access. When set: WebSocket sessions stay inert until the client answers the challenge (`authreq:<challenge>` → `auth:<hex sha256(password+challenge)>`; 3 attempts per connection; plaintext fallback for legacy tools; constant-time compare); `/input` HTTP injection requires a matching `pw=` parameter (403 otherwise); the unauthenticated legacy binary path is refused entirely; the web client shows a password overlay and re-syncs its settings after `auth:ok`. The password never crosses the wire in plain text when the challenge flow is used.
